...
class Surat_masuk extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['judul'] = "SIM - Permohonan Surat";
    ...
    public function detail()
    {
        ...
        $data['user'] = $this->db->get_where('detail_user', ['nim' => $this->session->userdata('nim')])->row_array();
        $data['akun'] = $this->db->get_where('user', ['nim' => $this->session->userdata('nim')])->row_array();
        $data['list_surat'] = $this->model_detail_surat->daftar_surat($data['detail']['jenis_surat'], $data['detail']['prodi'])->result(); 
        // echo $data['list_surat'][0]->no_surat;

        $judul = "Detail Surat Masuk";
        $data['judul'] = $judul;
        // echo $data['detail']['jenis_surat'];
        // echo $data['detail']['prodi'];
